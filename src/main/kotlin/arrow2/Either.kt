...
sealed class Either<out Error, out Value> {
  ...
  // Replaced by catch ??
  inline fun getOrHandle(crossinline default: (error: Error) -> @UnsafeVariance Value): Value =
    catch<Nothing> { default(it) }.get()
  
  @OptIn(ExperimentalTypeInference::class)
  @BuilderInference
  inline fun <Error2> catch(
    // Doesn't allow foreign suspension due to `EagerEffectScope`
    crossinline resolve: suspend EagerEffectScope<Error2>.(Error) -> @UnsafeVariance Value,
  ): Either<Error2, Value> = when (this) {
    is Right -> this
    is Left -> eagerEffect {
      resolve(error)
  ...
  //   // Doesn't allow foreign suspension due to `EagerEffectScope`
  //   resolve: suspend EffectScope<Error2>.(Error) -> @UnsafeVariance Value,
  // ): Either<Error2, Value> = when (this) {
  //   is Right -> this
  //   is Left -> effect {
  //     resolve(error)
...
// Renamed from `merge`
fun <A> Either<A, A>.get(): A = when (this) {
...
operator fun <A : Comparable<A>, B : Comparable<B>> Either<A, B>.compareTo(other: Either<A, B>): Int = when (this) {
 ...
 */
public fun <E, A, B> Either<E, A>.zip(SE: Semigroup<E>, fb: Either<E, B>): Either<E, Pair<A, B>> = zip(SE, fb, ::Pair)
...
public inline fun <E, A, B, C, D, EE, F, G, H, I, J, Z> Either<E, A>.zip(
  ...
  if (this is Either.Right && b is Either.Right && c is Either.Right && d is Either.Right && e is Either.Right && ff is Either.Right && g is Either.Right && h is Either.Right && i is Either.Right && j is Either.Right) {
    ...
    accumulatedError = if (this@zip is Either.Left) this@zip.error else accumulatedError
    accumulatedError = if (b is Either.Left) emptyCombine(accumulatedError, b.error) else accumulatedError
    accumulatedError = if (c is Either.Left) emptyCombine(accumulatedError, c.error) else accumulatedError
    accumulatedError = if (d is Either.Left) emptyCombine(accumulatedError, d.error) else accumulatedError
    accumulatedError = if (e is Either.Left) emptyCombine(accumulatedError, e.error) else accumulatedError
    accumulatedError = if (ff is Either.Left) emptyCombine(accumulatedError, ff.error) else accumulatedError
    accumulatedError = if (g is Either.Left) emptyCombine(accumulatedError, g.error) else accumulatedError
    accumulatedError = if (h is Either.Left) emptyCombine(accumulatedError, h.error) else accumulatedError
    accumulatedError = if (i is Either.Left) emptyCombine(accumulatedError, i.error) else accumulatedError
    accumulatedError = if (j is Either.Left) emptyCombine(accumulatedError, j.error) else accumulatedError
